from ophyd import Device, Component as Cpt, FormattedComponent as FCpt, EpicsSignal, EpicsSignalRO, EpicsMotor
...
class NHQ203MChannel(Device):
    ...
    status = FCpt(EpicsSignalRO, name="status",
                  suffix="{prefix}:ModStatus{ch_num}_rbv")
...
def load_power_supplies(config=None):
    ...
    for name, ps_config in ps_configs.items():
        ...
        for ch_num in range(1, ps_config['n_channels'] + 1):
            ...
            NHQ203MChannel(name=this_name, prefix=ps_config['prefix'], ch_num=ch_num,
