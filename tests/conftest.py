...
def ioc_undulator(request):
    ...
    return run_fake_ioc(module_name="haven.tests.ioc_undulator",
...
def ioc_bss(request):
    prefix = "255idc:bss:"
    pvs = dict(esaf_id=f"{prefix}esaf:id",
               esaf_cycle=f"{prefix}esaf:cycle",
               proposal_id=f"{prefix}proposal:id",
    )
    return run_fake_ioc(module_name="haven.tests.ioc_apsbss",
...
def run_fake_ioc(module_name, prefix: str, request, name="Fake IOC", pvs=None, pv_to_check: str=None):
    ...
    if pv_to_check is not None:
        ...
        poll_readiness(pv_to_check, timeout=poll_timeout,
                       attempts=poll_attempts, process=process)
...
def ioc_scaler(request):
    ...
    return run_fake_ioc(module_name="haven.tests.ioc_scaler",
...
def ioc_ptc10(request):
    prefix = "255idptc10:"
    pvs = dict(pid1_voltage=f"{prefix}5A:output",
               pid1_voltage_rbv=f"{prefix}5A:output_RBV",
               tc1_temperature=f"{prefix}2A:temperature"
    )
    return run_fake_ioc(module_name="haven.tests.ioc_ptc10",
...
def ioc_motor(request):
    prefix = "255idVME:"
    pvs = dict(m1=f"{prefix}m1",
               m2=f"{prefix}m2",
               m3=f"{prefix}m3",
               m4=f"{prefix}m4")
    return run_fake_ioc(module_name="haven.tests.ioc_motor",
                        name="Fake motor IOC",
                        prefix=prefix,
                        pvs = pvs,
                        pv_to_check=pvs["m1"],
...
def ioc_preamp(request):
    ...
    return run_fake_ioc(module_name="haven.tests.ioc_preamp",
...
def ioc_simple(request):
    ...
    pv_to_check = pvs["A"]
    return run_fake_ioc(module_name="haven.tests.ioc_simple",
...
def ioc_mono(request):
    prefix = "255idMono:"
    pvs = dict(bragg=f"{prefix}ACS:m3",
               energy=f"{prefix}Energy",
               id_tracking=f"{prefix}ID_tracking",
               id_offset=f"{prefix}ID_offset")
    return run_fake_ioc(module_name="haven.tests.ioc_mono",
                        name="Fake mono IOC",
                        prefix=prefix,
                        pvs = pvs,
                        pv_to_check=pvs["energy"],
