...
def test_pseudo_to_real_positioner(ioc_mono, ioc_undulator):
    positioner = EnergyPositioner(name="energy",
                                  mono_energy_pv="mono_ioc:Energy",
                                  id_prefix="id_ioc")
    ...
    assert positioner.get().id_energy == 5100
...
def test_real_to_pseudo_positioner(ioc_mono, ioc_undulator):
    positioner = EnergyPositioner(name="energy",
                                  mono_energy_pv="mono_ioc:Energy",
                                  id_prefix="id_ioc")
    positioner.wait_for_connection(timeout=10.)
    # Move the mono energy positioner
    epics.caput("mono_ioc:Energy", 5000.)
    time.sleep(0.1)  # Caproto breaks pseudopositioner status
    assert epics.caget("mono_ioc:Energy.VAL") == 5000.
    assert epics.caget("mono_ioc:Energy.RBV") == 5000.
    # Check that the pseudo single is updated
    assert positioner.energy.get().readback == 5000.
