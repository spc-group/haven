...
def test_moves_energy(mono_motor, id_motor, ion_chamber, pitch2_motor):
    ...
    fit_model = MagicMock()
    RE = RunEngineStub()
    RE(mono_ID_calibration(energies=[8000], fit_model=fit_model))
    assert id_motor.readback.get() == 8000


def test_aligns_pitch(mono_motor, id_motor, ion_chamber, pitch2_motor):
    fit_model = MagicMock()
    plan = mono_ID_calibration(energies=[8000], fit_model=fit_model)
    messages = list(plan)
