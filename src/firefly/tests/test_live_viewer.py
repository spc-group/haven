...
Message = namedtuple(
    "Message", ("topic", "partition", "offset", "key", "timestamp", "value")
)
...
messages = [
    Message(
        **{
            "topic": "s25idc_queueserver-dev",
            "partition": 0,
            "offset": 4,
            "key": b"kafka-unit-test-key",
            "timestamp": 1717042074437,
            "value": (
                [
                    "start",
                    {
                        ...
                        "configuration": {
                            "I0": {
                                "data": {
                                    ...
                                    "I0_voltmeter_input_link": "@asyn(LJT7V_0 "
                                    "2)ANALOG_IN_VALUE",
...
def display(affapp, catalog, event_loop):
    # Mock the kafka consumer with documents from 25-ID-C
    consumer = asynctest.MagicMock()
    consumer.__aiter__.return_value = messages
    consumer.start = AsyncMock()
    consumer.stop = AsyncMock()
    # Create the display object
    display = LiveViewerDisplay(root_node=catalog, kafka_consumer=consumer)
    # Mock the actual database functions
    display.db = AsyncMock()
    display.db.signal_names.return_value = (["energy_energy"], ["y"])
    display.db.metadata.return_value = {
        "": {"start": {"hints": {"dimensions": [[["energy_energy"], "primary"]]}}}
    }
    display.db.signals.return_value = {}
    display.db.all_signals.return_value = {}
    # Yield display to run the test
    try:
        yield display
    finally:
        # time.sleep(1)
...
@pytest.mark.asyncio
async def test_kafka_client(display, qtbot):
    assert hasattr(display, "kafka_consumer")
    # qtbot.wait(500)  # Give the consumer a chance to run in the event loop
    display.update_1d_plot.assert_called_once()
